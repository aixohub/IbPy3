...
class BarData(Object):
    ...
    def __str__(self):
        return "Date: %s, Open: %s, High: %s, Low: %s, Close: %s, Volume: %s, WAP: %s, BarCount: %s" % (
            self.date, ibapi.utils.floatMaxString(self.open),
            ibapi.utils.floatMaxString(self.high), ibapi.utils.floatMaxString(self.low),
            ibapi.utils.floatMaxString(self.close),
            ibapi.utils.decimalMaxString(self.volume), ibapi.utils.decimalMaxString(self.wap),
            ibapi.utils.intMaxString(self.barCount))


class RealTimeBar(Object):
    def __init__(self, time=0, endTime=-1, open_=0., high=0., low=0., close=0., volume=UNSET_DECIMAL, wap=UNSET_DECIMAL,
                 count=0):
        self.time = time
    ...
    def __str__(self):
        return "Time: %s, Open: %s, High: %s, Low: %s, Close: %s, Volume: %s, WAP: %s, Count: %s" % (
            ibapi.utils.intMaxString(self.time),
            ibapi.utils.floatMaxString(self.open), ibapi.utils.floatMaxString(self.high),
            ibapi.utils.floatMaxString(self.low),
            ibapi.utils.floatMaxString(self.close), ibapi.utils.decimalMaxString(self.volume),
            ibapi.utils.decimalMaxString(self.wap),
...
class PriceIncrement(Object):
    ...
    def __str__(self):
        return "LowEdge: %s, Increment: %s" % (
            ibapi.utils.floatMaxString(self.lowEdge), ibapi.utils.floatMaxString(self.increment))
...
class HistoricalTick(Object):
    ...
    def __str__(self):
        return "Time: %s, Price: %s, Size: %s" % (
            ibapi.utils.intMaxString(self.time), ibapi.utils.floatMaxString(self.price),
            ibapi.utils.decimalMaxString(self.size))
...
class HistoricalTickBidAsk(Object):
    ...
    def __str__(self):
        return "Time: %s, TickAttriBidAsk: %s, PriceBid: %s, PriceAsk: %s, SizeBid: %s, SizeAsk: %s" % (
            ibapi.utils.intMaxString(self.time), self.tickAttribBidAsk,
...
class HistoricalTickLast(Object):
    ...
    def __str__(self):
        return "Time: %s, TickAttribLast: %s, Price: %s, Size: %s, Exchange: %s, SpecialConditions: %s" % (
            ibapi.utils.intMaxString(self.time), self.tickAttribLast,
            ibapi.utils.floatMaxString(self.price), ibapi.utils.decimalMaxString(self.size), self.exchange,
            self.specialConditions)
...
class WshEventData(Object):
    ...
    def __str__(self):
        return "WshEventData. ConId: %s, Filter: %s, Fill Watchlist: %d, Fill Portfolio: %d, Fill Competitors: %d" % (
            ibapi.utils.intMaxString(self.conId),
