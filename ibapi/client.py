...
class EClient(object):
    ...
    def placeOrder(self, orderId: OrderId, contract: Contract, order: Order):
        ...
        if self.serverVersion() < MIN_SERVER_VER_ADVANCED_ORDER_REJECT and order.advancedErrorOverride:
            self.wrapper.error(orderId, UPDATE_TWS.code(),
                               UPDATE_TWS.msg() + "  It does not support advanced error override attribute")
            return
        ...
        try:
            ...
            if self.serverVersion() >= MIN_SERVER_VER_SMART_COMBO_ROUTING_PARAMS and contract.secType == "BAG":
                smartComboRoutingParamsCount = len(
                    order.smartComboRoutingParams) if order.smartComboRoutingParams else 0
    ...
    def cancelOrder(self, orderId: OrderId, manualCancelOrderTime: str):
        ...
        if self.serverVersion() < MIN_SERVER_VER_MANUAL_ORDER_TIME and manualCancelOrderTime:
            self.wrapper.error(orderId, UPDATE_TWS.code(),
                               UPDATE_TWS.msg() + "  It does not support manual order cancel time attribute")
            return
    ...
    def cancelPositionsMulti(self, reqId: int):
        ...
        msg = make_field(OUT.CANCEL_POSITIONS_MULTI) + make_field(VERSION) + make_field(reqId)
        self.sendMsg(msg)
    ...
    def cancelAccountUpdatesMulti(self, reqId: int):
        ...
        msg = make_field(OUT.CANCEL_ACCOUNT_UPDATES_MULTI) + make_field(VERSION) + make_field(reqId)
        self.sendMsg(msg)
...
def replaceFA(self, reqId: TickerId, faData: FaDataType, cxml: str):
    ...
    try:
        ...
        msg = make_field(OUT.REPLACE_FA) + make_field(VERSION) + make_field(int(faData)) + make_field(cxml)
...
def reqWshEventData(self, reqId: int, wshEventData: WshEventData):
    ...
    if self.serverVersion() < MIN_SERVER_VER_WSH_EVENT_DATA_FILTERS_DATE:
        if wshEventData.startDate != "" or wshEventData.endDate != "" or wshEventData.totalLimit != UNSET_INTEGER:
            self.wrapper.error(NO_VALID_ID, UPDATE_TWS.code(),
                               UPDATE_TWS.msg() + " It does not support WSH event data date filters.")
            return
