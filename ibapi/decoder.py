...
class Decoder(Object):
    ...
    def processContractDataMsg(self, fields):
        ...
        if version >= 5:
            contract.longName = decode(str, fields).encode().decode(
                'unicode-escape') if self.serverVersion >= MIN_SERVER_VER_ENCODE_MSG_ASCII7 else decode(str, fields)
            ...
            if pname != "self":
                ...
                try:
                    arg = fields[fieldIdx].decode(
                        'unicode-escape' if self.serverVersion >= MIN_SERVER_VER_ENCODE_MSG_ASCII7 else 'UTF-8')
